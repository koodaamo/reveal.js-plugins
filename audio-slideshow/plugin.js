...
const initAudioSlideshow = function (Reveal) {
  ...
  var subTitleURL = null; // optional webvtt subtitles
  var subTitleStyle = ""; // styling - if no positioning is given, we put subtitles above player controls
  ...
  Reveal.addKeyBinding(
    { keyCode: 65, key: "A", description: "Toggle audio" },
    function () {
      if (currentAudio) {
        if (timer) {
          clearTimeout(timer);
  ...
  function selectAudio(previousAudio) {
    ...
    if (currentAudio) {
      ...
      setCurrentSubtitleTrack(currentAudio);
      if (previousAudio) {
      ...
      // No audio element can exist when defaultDuration is 0 and no sources are found.
  ...
  function setup() {
    ...
    if (config) {
      ...
      if (config.subTitleURL != null) subTitleURL = config.subTitleURL;
      if (config.subTitleStyle != null) subTitleStyle = config.subTitleStyle;
      if (config.hideSubtitlesOnPause != null)
        hideSubtitlesOnPause = config.hideSubtitlesOnPause;
    }

    var subtitleConfig = revealConfig.subtitles;
    if (subtitleConfig) {
      if (subtitleConfig.subTitleURL != null)
        subTitleURL = subtitleConfig.subTitleURL;
      if (subtitleConfig.subTitleStyle != null)
        subTitleStyle = subtitleConfig.subTitleStyle;
      if (subtitleConfig.hideSubtitlesOnPause != null)
        hideSubtitlesOnPause = subtitleConfig.hideSubtitlesOnPause;
    }
    ...
    setupSubtitleDisplay(divElement);
  ...
  function setupAllAudioElements(container, h, v, slide) {
    if (slide.querySelector("code.fragment:not([data-fragment-index])")) {
      ...
      setupAudioElement(
        ...
        "",
        null,
        slide.getAttribute("data-audio-subtitle"),
      );
  ...
  function setupFallbackAudio(audioElement, text, videoElement) {
    ...
    if (textToSpeechURL != null && text != null && text != "") {
      ...
      if (textToSpeechURL.includes("[TEXT]")) {
        audioSource.src = textToSpeechURL.replace(
          "[TEXT]",
  ...
  function setupAudioElement(
    ...
    setupAudioSubtitles(audioElement, indices, subtitleURL);
  ...
  function setupSubtitleDisplay(container) {
    if (subtitleDisplay) return;
    subtitleDisplay = document.createElement("div");
    subtitleDisplay.className = "audio-subtitles";
    subtitleDisplay.setAttribute("aria-live", "polite");
    subtitleDisplay.setAttribute("style", buildSubtitleStyle());
    container.appendChild(subtitleDisplay);
  }

  function buildSubtitleStyle() {
    var baseStyle =
      "display: none; opacity: 0; padding: 6px 10px; border-radius: 6px; " +
      "background: rgba(0,0,0,0.75); color: #fff; " +
      "font-family: Arial, Helvetica, sans-serif; font-size: 18px; " +
      "line-height: 1.35; text-align: center; max-width: 100%; " +
      "pointer-events: none; transition: opacity 120ms ease;";
    var hasPositioning = /(^|;)\s*(position|top|bottom|left|right)\s*:/.test(
      (subTitleStyle || "").toLowerCase(),
    );
    var positionStyle = hasPositioning
      ? ""
      : "position: absolute; left: 0; right: 0; bottom: 80px;";
    return baseStyle + positionStyle + (subTitleStyle || "");
  }

  function resolveSubtitleURL(indices, overrideURL) {
    var url = overrideURL || subTitleURL;
    if (!url) return null;
    if (url.indexOf("[INDEX]") !== -1) {
      return url.replace("[INDEX]", indices);
    }
    return url;
  }

  function setupAudioSubtitles(audioElement, indices, overrideURL) {
    var subtitleURL = resolveSubtitleURL(indices, overrideURL);
    if (!subtitleURL) return;

    var trackElement = document.createElement("track");
    trackElement.kind = "metadata";
    trackElement.src = subtitleURL;
    trackElement.setAttribute("data-subtitle-track", audioElement.id);
    audioElement.appendChild(trackElement);

    var textTrack = trackElement.track;
    textTrack.mode = "hidden";
    audioElement._subtitleTrack = textTrack;

    textTrack.oncuechange = function () {
      if (audioElement !== currentAudio) return;
      renderSubtitle(textTrack, false);
    };
  }
